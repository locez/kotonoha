...
from PyQt6.QtGui import QColor, QFont, QGuiApplication, QMouseEvent, QPainter, QPaintEvent, QScreen, QShowEvent
...
class LyricsOverlay(QWidget):
    ...
    def __init__(self, state: LyricsState, config: Config, controller: LayerShellController | None = None) -> None:
        ...
        self._drag_global = QPointF()
        self._drag_panel_pos = QPointF()
        self._drag_origin_global = QPointF()
        self._drag_moved = False
        self._suppress_control_click = False
        self._layer_screen: QScreen | None = None
        self._layer_screen_bound = False
    ...
    def _build_control_bar(self) -> QWidget:
        ...
        self._settings_btn.clicked.connect(self._on_settings_button_clicked)
    ...
    def _on_lock_button_clicked(self) -> None:
        if self._finish_control_drag_if_needed():
            return
    ...
    def _finish_control_drag_if_needed(self) -> bool:
        """Keep a drag release retargeted to a moving button from becoming a click."""
        if self._dragging and self._drag_moved:
            self._finish_drag()
        if self._suppress_control_click:
            logger.debug("Ignoring control click generated by a completed overlay drag")
            return True
        return False
    ...
    def _target_screen(self):
        ...
        if not self._controller.available:
            # A normal X11/fallback window can genuinely move between outputs;
            # follow Qt's current screen in that case.
            return current

        # A layer-shell surface remains assigned to the output on which it was
        # created. Near an output edge Qt may nevertheless change QWindow.screen()
        # based on the surface's nominal rectangle. Using that other screen's
        # dimensions for margins moves the still-bound layer surface off its real
        # output and makes its input region unreachable until restart.
        screens = QApplication.screens()
        if self._layer_screen not in screens:
            # Before a layer surface is mapped, QWidget.screen() can follow the
            # output containing the pointer. The compositor assigns an otherwise
            # unspecified layer surface to the primary output, so choose that same
            # output explicitly instead of calculating margins for an adjacent
            # output while the compositor places the surface on the primary one.
            primary = QApplication.primaryScreen()
            self._layer_screen = primary if primary in screens else current
    ...
    def _apply_window_geometry(self) -> None:
        ...
        # Saved margins may come from another output/scale or from an older buggy
        # drag. Activate the layout first so the painted panel, rather than only
        # its much larger transparent surface, can be kept reachable on startup.
        self._root.activate()
    ...
    def _window_ptr(self) -> int | None:
        ...
        if handle is None:
            return None
        if self._controller.available and not self._layer_screen_bound:
            screen = self._target_screen()
            if screen is not None and handle.screen() is not screen:
                # Keep QWindow, LayerShellQt and our screen-local margins bound to
                # one output from the first map. Without this, the compositor may
                # keep the layer on one output while Qt reports an adjacent one.
                handle.setScreen(screen)
            self._layer_screen_bound = True
    ...
    # its margins. Local coordinates change when the compositor repositions that
    # surface. Adding local deltas to already-updated margins works only when every
    # move is immediately presented; an asynchronous compositor can feed the same
    # displacement back repeatedly. Instead, integrate event-global pointer deltas
    # into the desired position of the *visible panel*. The global origin is
    # irrelevant, and clamping that desired position on every event discards
    # overdrag immediately, so reversing by one pixel at an edge moves by one pixel.

    def mousePressEvent(self, a0: QMouseEvent | None) -> None:
        if a0 is not None and not self._passthrough and a0.button() == Qt.MouseButton.LeftButton:
            self._dragging = True
            self._drag_global = QPointF(a0.globalPosition())
            self._drag_panel_pos = QPointF(self._layer_pos + self._container.pos())
            self._drag_origin_global = QPointF(a0.globalPosition())
            self._drag_moved = False
    ...
    def mouseMoveEvent(self, a0: QMouseEvent | None) -> None:
        if a0 is not None and self._dragging and a0.buttons() & Qt.MouseButton.LeftButton:
            total_diff = a0.globalPosition() - self._drag_origin_global
            hints = QApplication.styleHints()
            threshold = hints.startDragDistance() if hints is not None else 10
            if self._drag_moved or total_diff.manhattanLength() >= threshold:
                self._drag_moved = True
                pointer = a0.globalPosition()
                diff = pointer - self._drag_global
                self._drag_global = QPointF(pointer)
                panel_offset = self._container.pos()
                # niri can reduce the configured width as a positive left margin
                # approaches the output edge. A centered panel then shifts left
                # inside that shrinking transparent surface. Track and clamp the
                # panel in output coordinates first, then derive the layer origin
                # from its latest local offset. This compensates the resize without
                # mixing compositor feedback into subsequent pointer deltas.
                self._drag_panel_pos = self._clamp_drag_panel_position(self._drag_panel_pos + diff)
                target = (self._drag_panel_pos - QPointF(panel_offset)).toPoint()
                if target != self._layer_pos:
                    self._layer_pos = target
    ...
    def _finish_drag(self) -> None:
        """Finish and persist a gesture, then restore its native input region."""
        ...
        # niri can reconfigure/crop a layer surface when margins put part of its
        # transparent band outside the output. Re-submit the pill's local input
        # rectangle after the final move; toggling passthrough used to be the only
        # action that repaired it, making a completed drag look permanently locked.
        self._apply_input_region()
        self._suppress_control_click = True
        QTimer.singleShot(0, self._clear_control_click_suppression)
    ...
    def _clamp_to_screen(self, pos: QPoint) -> QPoint:
        ...
        if panel.isValid() and panel.width() > 0 and panel.height() > 0:
            # Clamp the painted/input panel, not the transparent surface around it.
            # Otherwise 80 visible pixels of the surface can still mean that the
            # entire centered pill is off-screen and impossible to grab again.
            min_x = -panel.x()
    ...
    def _clamp_drag_panel_position(self, pos: QPointF) -> QPointF:
        """Clamp the visible panel in output coordinates during a live drag."""
        screen = self._target_screen()
        panel = self._container.geometry()
        if screen is None or not panel.isValid() or panel.width() <= 0 or panel.height() <= 0:
            panel_offset = self._container.pos()
            layer_pos = self._clamp_to_screen((pos - QPointF(panel_offset)).toPoint())
            return QPointF(layer_pos + panel_offset)
        geo = screen.geometry()
        x = max(0.0, min(pos.x(), float(max(0, geo.width() - panel.width()))))
        y = max(0.0, min(pos.y(), float(max(0, geo.height() - panel.height()))))
        return QPointF(x, y)

    def _apply_layer_position(self) -> None:
        """Apply the current position without forcing a costly Qt repaint."""
        if self._controller.available:
            ptr = self._window_ptr()
            if ptr is not None:
                self._controller.set_anchor_position(ptr, self._layer_pos.x(), self._layer_pos.y())
