...
Qt 6, PyQt6, Wayland, LayerShellQt, aiohttp, qasync, and dbus-fast are represented as system build/runtime dependencies rather than bundled Qt libraries. `dh-sequence-python3` activates `dh_python3` so `${python3:Depends}` is populated. The DEB build declares CMake and `python3-scikit-build-core` and lets pybuild invoke the project PEP 517 backend directly; scikit-build-core builds and installs the native bridge into the wheel alongside the Python package. This keeps the package build self-contained and links the bridge against the target distribution's Qt and LayerShellQt ABI without modifying the source tree.
...
The workflow uses the pinned `setup-uv` action with uv 0.11.19 only to run `uv version "$VERSION" --frozen`, creates a source archive named for the resolved version, updates both the spec `Version:` header and the single `%changelog` release to `${VERSION}-1`, and invokes `rpmbuild`. The spec declares CMake and `python3-scikit-build-core`; Fedora's Python wheel macro invokes the same backend used by local builds, which installs the native bridge into the wheel without source-tree staging.
